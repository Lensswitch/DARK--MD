  ...
  try {
    ...
    zk.awaitForMessage({
      ...
      try {
        ...
        repondre('*Téléchargement terminé.\nDARK vous remercie pour votre patience*');
        repondre('*Cette commande a été développée par Christian-packeur un développeur ivoirien*')
      }
    }).catch(error => {
      if (error.message == 'Timeout') {
        try {
          console.log('Timeout');
          zk.sendMessage(origineMessage, { text: '*Demande annulée, vous avez pris trop de temps avant de répondre DARK est déçu*', edit: sentMessage.key });
        } catch (err) {
          console.log(err);
        }
      } else {
